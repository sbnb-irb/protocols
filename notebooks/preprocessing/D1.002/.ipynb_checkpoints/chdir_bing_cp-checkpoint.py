...
# # This script will be ran by the HPC module
# # Which will provide it with the command line arguments task_id and file_name

task_id = sys.argv[1]  # <TASK_ID>
filename = sys.argv[2]  # <FILE>
root_LINCS = sys.argv[3] # path to metadata dictionaries
root_out = sys.argv[4] # path where chdir signatures are saved
level3_beta_trt_cp_n1805898x12328 = sys.argv[5] # path to cp data
level3_beta_ctl_n188708x12328 = sys.argv[6] # path to control data

print("TaksID is ", task_id)
print("Filename is ", filename)

input_pickle = pickle.load(open(filename, 'rb'))

cb = input_pickle[task_id][0]
...
batch2sig = pickle.load(open(root_LINCS + 'CP_batch2sig_id.p', 'rb'))
sig2inst = pickle.load(open(root_LINCS + 'CP_sig_id2distil_ids.p', 'rb'))
batch2inst = pickle.load(open(root_LINCS + 'CP_batch2inst_id.p', 'rb'))
ctr_batch2inst = pickle.load(open(root_LINCS + 'CTR_batch2inst.p', 'rb'))
...
def CD_replicate(selected_samples, controls_selected): 
    ...
    for i in plates:
        ...
        if sample_plate.shape[0] == control_plate.shape[0]: 
            exp_id = list(np.array(exp_id).flatten())
        else:        
            exp_id = list(np.concatenate(np.array(exp_id).flatten()))
...
ctr = ctr_batch2inst[cb]
exp_replicates = batch2inst[cb]
...
if significative: 
    ...
    with h5py.File(root_out + '%s_chdir.h5'%cb, "w") as o:
    ...
    with h5py.File(root_out + '%s_bck_cutoffs.h5'%cb, "w") as o:
        ...
        o.create_dataser('distribution', data = np.asarray([x[1] for x in rep2d]))


if no_rep: 
    with h5py.File(root_out + '%s_chdir_single.h5'%cb, "w") as o:
