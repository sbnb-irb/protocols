#!/usr/bin/env bash
# Runs compare_cc_spaces.py inside the CC Singularity image, replicating the
...
#
# Usage: mirrors compare_cc_spaces.py's own CLI -- pass its arguments through.
#
#   ./run_compare.sh \
...
if [[ ! -f "${CC_SIMG}" ]]; then
    ...
    echo "Override with CC_SIMG=/path/to/cc.simg ./run_compare.sh ..." >&2
    exit 1
fi
...
exec singularity exec --nv --cleanenv \
    -B "${RUN_USER_BIND}" \
    "${CC_SIMG}" \
    python "${SCRIPT_DIR}/compare_cc_spaces.py" "$@"